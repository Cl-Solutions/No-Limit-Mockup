    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-20">
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <button
              ...
              className="text-[#444444] dark:text-white/70 hover:text-[#111111] dark:hover:text-white text-sm font-medium tracking-wide transition-colors duration-150 relative group"
            >
              {link.label}
          ...
          <button
            onClick={toggleTheme}
            className="w-9 h-9 flex items-center justify-center rounded-full border border-black/10 dark:border-white/10 text-[#444444] dark:text-white/70 hover:text-[#111111] dark:hover:text-white hover:border-black/20 dark:hover:border-white/20 transition-[border-color,color] duration-150"
        ...
        <div className="lg:hidden flex items-center gap-3">
          <button
            onClick={toggleTheme}
            className="w-9 h-9 flex items-center justify-center rounded-full border border-black/10 dark:border-white/10 text-[#444444] dark:text-white/70 transition-[border-color,color] duration-150"
            aria-label="Theme umschalten"
          >
            {theme === 'dark' ? <Sun size={16} /> : <Moon size={16} />}
          </button>
            ...
            <div className="px-6 py-4 flex flex-col gap-1">
              {navLinks.map((link) => (
                <button
                  key={link.href}
                  onClick={() => handleNav(link.href)}
                  className="text-[#444444] dark:text-white/80 hover:text-[#111111] dark:hover:text-white text-base font-medium py-3 text-left border-b border-black/5 dark:border-white/5 transition-colors duration-150"
                >
                  {link.label}
                </button>
              ))}
